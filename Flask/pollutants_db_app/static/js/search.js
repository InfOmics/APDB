...
$(document).ready(function(){ 
  ...
  $('#mol_id_option').on('change', function() {
    ...
    if (found == true) {
      ...
      window.location.replace(`http://localhost:5000/apdb/home/browse-molecules?id_option=${SelectedOption}`);
    } else {
      // clear input text 
      $('#mol_id_option').val("");
    }
...
$(document).ready(function(){ 
  ...
  $('#bio_id_option').on('change', function() {
    ...
    if (found == true) {
      // create a url location with selected id option
      window.location.replace(`http://localhost:5000/apdb/home/browse-bioassays?id_option=${SelectedOption}`);
    } else {
      // clear input text 
      $('#bio_id_option').val("");
    }
    ...
    if(e.which == 13) {  
      if ($.trim($('textarea#molecule_id').val()) != "") {
        ...
        if (found == true) {
          ...
          window.location.replace(`http://localhost:5000/apdb/home/browse-similarities/panel?molecule_id=${SearchTerm}&similarity_thr=${threshold}`);
        } else {
